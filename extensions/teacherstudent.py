...
def train(cfg_dict, train_loader, val_loader):
    ...
    simvp = SimVPSegmentation(shape_in=(11, 3, IMG_SIZE[0], IMG_SIZE[1]))

    simvp = nn.DataParallel(simvp)
    ...
    start_epoch = 0
    if os.path.isfile(os.path.join(cfg_dict["model_root"], "st", cfg_dict["st_model_name"])):
        logging.info("Restarting training from checkpoint")
        checkpoint = torch.load(os.path.join(cfg_dict["model_root"], "st", cfg_dict["st_model_name"]))
            ...
            with torch.no_grad():
                ...
                if (avg_val_loss<best_avg_val_loss):
                    ...
                    torch.save(
                        ...
                        cfg_dict["model_root"] + "st/" + f"/model_{file_identifier}.pth",
                    )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename= cfg_dict['log_dir'] + 'st_train.log',filemode='a',level=logging.INFO)
